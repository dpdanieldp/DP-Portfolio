...
def commodities_b():
    ...
    zmiana_proc_s = [
        ((i.get_text(strip=True)).replace(",", ".")).replace("\xa0", "") for i in d
    ]
...
def stocks_b():
    ...
    kurs_a = [
        ((i.get_text(strip=True)).replace(",", ".")).replace("\xa0", "") for i in b
    ]
    zmiana_proc_a = [
        ((i.get_text(strip=True)).replace(",", ".")).replace("\xa0", "") for i in c
    ]
...
def bankier_info_to_db_currencies():
    ...
    if not date_in_db:
        last_call_date = models.LastCalls(
            source="bankier_date", last_call=avg_currencies["Last_call"]
        )
    ...
    for i in range(len(avg_currencies["Currency"])):
        currency_info = models.BankierPLNCurrency(
            currency=avg_currencies["Currency"][i],
...
def bankier_info_to_db_rest():
    ...
    if not date_in_db:
        last_call_date = models.LastCalls(
            source="bankier_minutes", last_call=commodities["Last_call"]
        )
        db.session.add(last_call_date)
    else:
    ...
    for i in range(len(commodities["Ticker"])):
        commodities_info = models.BankierCommodity(
            ticker=commodities["Ticker"][i],
    ...
    for i in range(len(stocks["Ticker"])):
        stocks_info = models.BankierStockBond(
    ...
    for i in range(len(new_connect["Ticker"])):
        new_connect_info = models.BankierStockBond(
            ticker=new_connect["Ticker"][i],
    ...
    for i in range(len(bonds["Ticker"])):
        bonds_info = models.BankierStockBond(
            ticker=bonds["Ticker"][i],
            name=bonds["Name"][i],
...
def bankier_rest_check_or_not():
    ...
    last_call_b_minutes = models.LastCalls.query.filter_by(
        source="bankier_minutes"
    ).first()

    if last_call_b_minutes:

        if now - (
            datetime.strptime(last_call_b_minutes.last_call, "%Y-%m-%d %H:%M")
