from . import polygon_check_tickers, bankier_btfs
...
def check_wallet_components(user, currency):
    ...
    if len(user_wallet) == 0:
            ...
            if component.category == 'Other':
                ...
                if component.value_currency != currency:
                    ...
                    if currency == 'PLN':
                        ...
                        old_currency_data = models.BankierPLNCurrency.query.filter_by(currency=old_currency).first()
                        old_currency_pln_rate = old_currency_data.rate
                        old_currency_pln_price = old_currency_data.pln_price

                        new_value = (old_value / old_currency_pln_rate) * old_currency_pln_price
                        ...
                        if old_currency == 'PLN':
                            new_value = (old_value * new_currency_pln_rate) / new_currency_pln_price
                        else:
                            old_currency_data = models.BankierPLNCurrency.query.filter_by(currency=old_currency).first()
                            old_currency_pln_rate = old_currency_data.rate
                            old_currency_pln_price = old_currency_data.pln_price
                            value_in_pln = (old_value / old_currency_pln_rate) * old_currency_pln_price
                            new_value = (value_in_pln * new_currency_pln_rate) / new_currency_pln_price
                ...
                if component.category == 'Polish stocks/bonds':
                    ...
                    ticker_data = models.BankierStockBond.query.filter_by(ticker=component.ticker).first()
                    ...
                    if currency == 'PLN':
                        ...
                        new_currency_ticker_price = (ticker_data.price * new_currency_pln_rate) / new_currency_pln_price
                ...
                if component.category == 'Cash':
                    ...
                    if component.ticker == 'PLN' and currency == 'PLN':
                        currency_data = models.BankierPLNCurrency.query.filter_by(id=1).first()
                ...
                if component.category == 'Commodities':
                    ...
                    ticker_data = models.BankierCommodity.query.filter_by(ticker=component.ticker).first()

                    usd_data = models.BankierPLNCurrency.query.filter_by(currency='USD').first()
                    usd_pln_rate = usd_data.rate
                    usd_pln_price = usd_data.pln_price

                    if currency == 'PLN':

                        component.value_per_1 = round(((ticker_data.price / usd_pln_rate) * usd_pln_price), 2)
                        component.value = round((component.value_per_1 * component.amount), 2)
                        component.date = ticker_data.date
                        db.session.commit()
                        ...
                        value_per_1_in_pln = (ticker_data.price / usd_pln_rate) * usd_pln_price

                        value_per_1_in_currency = (value_per_1_in_pln * new_currency_pln_rate) / new_currency_pln_price
                ...
                if component.category == 'Gold/Silver bullion':
                    ...
                    ticker_data = models.BankierCommodity.query.filter_by(ticker=ticker).first()
                    ...
                    if currency == 'PLN':
                        if component.ticker == 'Gold bullion':
                            ...
                            value1 = ((ticker_data.price / usd_pln_rate) * usd_pln_price) * 1.04
                            value = value1 * qty
                        ...
                        if component.ticker == 'Silver bullion':
                            value1 = ((ticker_data.price / usd_pln_rate) * usd_pln_price) * (1.28)
        ...
        if len(polygon_tickers_list) > 0:
            ...
            polygon_ticker_value_dict = polygon_check_tickers.polygon_check_or_not(polygon_tickers_list)
            ...
            for component in user_wallet:
                if component.ticker in response_tickers_list:
                    ...
                    if not component.name:
                        ticker_name = models.PolygonTicker.query.filter_by(ticker=component.ticker).first()
                    ...
                    if currency == 'PLN':
                        ...
                        value_per_1_in_pln = (polygon_ticker_value_dict[
                                                  component.ticker][0] / usd_pln_rate) * usd_pln_price
                        ...
                        component.value_per_1 = round(value_per_1_in_currency, 2)
                        component.value = round((value_per_1_in_currency * component.amount), 2)
                        component.date = polygon_ticker_value_dict[component.ticker][1]
                        db.session.commit()
...
def check_wallet_sum_value(user):
    ...
    if len(user_wallet) == 0:
        ...
        for component in user_wallet:
            component.percentage_in_wallet = round(((component.value / current_wallet_value) * 100), 2)
            db.session.commit()

        user_wallet_value = models.WalletValue.query.filter_by(user_id=user.id).first()
        if not user_wallet_value:
            current_value = models.WalletValue(sum_value=round(current_wallet_value, 2), user_id=user.id)
