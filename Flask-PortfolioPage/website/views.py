...
from extensions import bankier_btfs, polygon_tickersnames,\
    check_wallet, ti_wallet_report, gb_db_operations, gb_api_operations, sort_book_info
import json
import ast
import datetime
...
def gb_home():
    ...
    if request.method == 'POST':
        if request.form.get('search'):
            ...
            if set(fields) == set(empty_fields):
                ...
                if date2 < date1:
                    flash('The end date cannot be earlier than the start date', category='error')
                else:
                    ...
                    if response is False:
                        ...
                        return render_template("gbhome.html", empty_table=gb_db_operations.check_if_empty_table(),
                                               dates=sorted(dates2),
    ...
    return render_template("gbhome.html", empty_table=gb_db_operations.check_if_empty_table(), dates=sorted(dates2),
                           books=books, user=current_user)
        ...
        if request.form.get('addBook'):
            ...
            if '' in set(list_of_required_fields):
                ...
                if info_from_db is False:
                    ...
                    books = Book.query.all()
                    return render_template("add_book.html", empty_table=gb_db_operations.check_if_empty_table(),
                                           books=books, user=current_user)
                else:
                    ...
                    gb_db_operations.save_temporary_info(key='l_isbn', info=str([clean_isbn]))
                    gb_db_operations.save_temporary_info(key='books_info_d', info=str(books_info_d))
                    gb_db_operations.save_temporary_info(key='old_id', info='-')
                    return redirect((url_for('views.confirm_overwriting')))

    return render_template("add_book.html", empty_table=gb_db_operations.check_if_empty_table(), books=books,
                           user=current_user)
...
def confirm_overwriting():
    ...
    if books_info_str is None or l_isbn_str is None:
        ...
        return render_template("confirm_overwriting.html", data_in_db=data_in_db, books_info_d=books_info_d,
                               lenght=len(books_info_d["add_title"]), user=current_user)
...
def import_book():
    ...
    if request.method == 'POST':
        if request.form.get('searchInGB'):
            ...
            if set(list(query_d.values())) == {''}:
                ...
                if response is False:
                    info = 'error'
                    flash("An error occurred while connecting to Google Books", category='error')
                    return render_template("import_from_API.html", empty_table=gb_db_operations.check_if_empty_table(),
                                           info=info, books=books, user=current_user)
                ...
                if db_info is False:
                    ...
                    if len(db_info) < len(response["add_title"]):
                        ...
                        try:
                            ...
                            gb_db_operations.save_temporary_info(key='l_isbn',
                                                              info=str(sorted_book_info[0]["clean_isbn"]))

                            gb_db_operations.save_temporary_info(key='books_info_d',
    ...
    return render_template("import_from_API.html", empty_table=gb_db_operations.check_if_empty_table(),
                           info='before_result', books=books, user=current_user)
...
def api_request():
    ...
    if gb_db_operations.check_if_empty_table() is True or set(arguments_l) in invalid_sets or correct_date is False:
        ...
        if response is False:
            ...
            return (jsonify({'results': str(len(response)), 'api_response': books_schema.dump(response)}), 200)
...
def ti_wallet():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('OKchangeCurrency'):
            ...
            if not empty_wallet:
                check_wallet.check_wallet_components(user=current_user, currency=currency)

            current_wallet_value = check_wallet.check_wallet_sum_value(user=current_user)

            currencies = check_wallet.get_cur_list(user=current_user)

            return render_template("ti_home.html", user=current_user, empty_wallet=empty_wallet, currencies=currencies,
        ...
        if request.form.get('addComponent'):
            ...
            list_of_categories = ['Polish stocks/bonds', 'US stocks/ ETFs', 'Gold/Silver bullion',
                                  'Commodities', 'Crypto',
                                  'Cash', 'Other']

            return render_template("ti_add_comp1.html", user=current_user, empty_wallet=empty_wallet,
                                   list_of_categories=list_of_categories, currencies=currencies)
        ...
        if request.form.get('OKcategory'):
            ...
            if category == 'Select category...':
                ...
                list_of_categories = ['Polish stocks/bonds', 'US stocks/ ETFs', 'Gold/Silver bullion', 'Commodities',
                                      'Crypto',
                                      'Cash', 'Other']

                return render_template("ti_add_comp1.html", user=current_user,
                                       empty_wallet=empty_wallet, currencies=currencies,
                ...
                if category == 'US stocks/ ETFs':
                    tickers_names = PolygonTicker.query.filter(not_(PolygonTicker.ticker.like('%C:%'))).filter(
                        not_(PolygonTicker.ticker.like('%X:%'))).all()
                    tickers = []
                    names = []
                    for result in tickers_names:
                        tickers.append(result.ticker)
                ...
                return render_template("ti_add_comp2_stocksbonds.html", user=current_user,
                                       empty_wallet=empty_wallet, category=category,
                                       zipped_tickers_names=zipped_tickers_names,
                                       currencies=currencies, current_wallet_value=current_wallet_value)

            elif category in ['Commodities', 'Crypto', 'Cash',
                              'Gold/Silver bullion']:

                if category == 'Crypto':
                    tickers_names = PolygonTicker.query.filter(PolygonTicker.ticker.like('%X:%')).filter(
                        PolygonTicker.ticker.like('%USD%')).all()
                    tickers = []
                    names = []
                    for result in tickers_names:
                        tickers.append(result.ticker)
                ...
                if category == 'Cash':
                    ...
                    zipped_tickers_names = zip(sorted(tickers_with_pln), sorted(names_with_pln))
                ...
                return render_template("ti_add_comp2_ccc.html", user=current_user,
                                       empty_wallet=empty_wallet, category=category,
                                       zipped_tickers_names=zipped_tickers_names,
                ...
                return render_template("ti_add_comp2_other.html", user=current_user,
                                       empty_wallet=empty_wallet, category=category,
                                       currencies=currencies)
        ...
        if request.form.get('addThisComponent'):
            ...
            if category == 'Other':
                ...
                if (asset is None) or (value is None):
                    ...
                    component = Wallet(ticker=asset, name=asset, category=category, amount=quantity, value_per_1=value,
                                       value=value, value_currency=currency, date=component_date,
                    ...
                    return render_template("ti_add_comp1.html", user=current_user,
                                           empty_wallet=empty_wallet, list_of_categories=list_of_categories,
                                           currencies=currencies)
                ...
                if (asset == 'Select asset...') or (quantity is None):
                    ...
                    component = Wallet(ticker=asset_ticker, category=category, amount=quantity, user_id=current_user.id)
