    ...
    if request.method == 'POST':
        ...
        if user:
            if user.email_confirmed:
                if check_password_hash(user.password, password) and user.email_confirmed:
                ...
                flash('Your email is not confirmed.'
                      ...
                      'Follow the link to complete your account registration.', category='error')
...
def send_confirmation_email(email):
    ...
    msg = Message('TI: Confirm your email',
...
def send_change_password_email(email):
    token = ts.dumps(email, salt='email-confirm-key')
    msg = Message('TI: Change password',
                  sender=('Treasure Island', 'ti.treasureisland.mail@gmail.com'),
                  recipients=[email])
...
def reset_token(token):
    ...
    return render_template('change_password.html', title='Change Password', user=current_user)
