...
def gb_query(query_d):
    ...
    google_key = 'YOUR_GOOGLE_BOOKS_API_KEY'
    country_name = 'PL'
    max_results = 40
    start_index = 0
    ...
    if response.status_code != 200:
        ...
        if int(data['totalItems']) <= len(data['items']):
            ...
            for response in response_l:
                added_info_d2 = add_response_to_dict(books_info_d=added_info_d, response=response)
...
def add_response_to_dict(books_info_d, response):
    for i in range(len(response['items'])):
        ...
        try:
            books_info_d['add_title'].append(response['items'][i]['volumeInfo']['title'])
        except KeyError:
            books_info_d['add_title'].append('!REMOVE_ME!')
        finally:
            try:
                books_info_d['add_author'].append(';'.join(response['items'][i]['volumeInfo']['authors']))
            except KeyError:
                books_info_d['add_author'].append('---')
            finally:
                try:
                    books_info_d['add_pub_date'].append(response['items'][i]['volumeInfo']['publishedDate'])
                except KeyError:
                    books_info_d['add_pub_date'].append('---')
                finally:

                    try:
                        isbns_list_of_dicts = response['items'][i]['volumeInfo']['industryIdentifiers']
                        ...
                        try:
                            books_info_d['add_page_count'].append(response['items'][i]['volumeInfo']['pageCount'])
                        except KeyError:
                            ...
                            try:
                                ...
                                try:
                                    books_info_d['add_language'].append(response['items'][i]['volumeInfo']['language'])
                                except KeyError:
...
def remove_duplications(d_without_invalid):
    ...
    if number_of_all_positions == number_of_unique_positions:
        ...
        d_without_duplications = {"add_title": [],
                                  "add_author": [],
                                  "add_pub_date": [],
                                  "clean_isbn": [],
                                  "add_page_count": [],
        ...
        if '---' in unique_isbn:
            unique_isbn.remove('---')
            indices_of_no_isbn = [i for i, x in enumerate(d_without_invalid['clean_isbn']) if x == "---"]
            unique_titles_without_isbn = []
            for idx in indices_of_no_isbn:
                if d_without_invalid['add_title'][idx] not in unique_titles_without_isbn:
                    indices_of_unique_titles_without_isbn.append(idx)
                    unique_titles_without_isbn.append(d_without_invalid['add_title'][idx])

        for isbn in unique_isbn:
            idx = d_without_invalid['clean_isbn'].index(isbn)
            d_without_duplications['add_title'].append(d_without_invalid['add_title'][idx])
            d_without_duplications['add_author'].append(d_without_invalid['add_author'][idx])
            d_without_duplications['add_pub_date'].append(d_without_invalid['add_pub_date'][idx])
            d_without_duplications['clean_isbn'].append(d_without_invalid['clean_isbn'][idx])
            d_without_duplications['add_page_count'].append(d_without_invalid['add_page_count'][idx])
            d_without_duplications['add_link_to_cover'].append(d_without_invalid['add_link_to_cover'][idx])
            d_without_duplications['add_language'].append(d_without_invalid['add_language'][idx])
