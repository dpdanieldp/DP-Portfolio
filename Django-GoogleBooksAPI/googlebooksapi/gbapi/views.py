...
def home(request):
            ...
            if set(fields) == set(empty_fields):
                ...
                if date2 < date1:
                    ...
                    if response is False:
                        messages.error(
                            request,
                            f"Brak wyników dla kryteriów: Tytuł: {title}, Autor: {author},"
    ...
    return render(
        request,
        "gbapi/home.html",
        {"empty_table": empty_table, "dates": dates2, "books": books},
    )
        ...
        if request.POST.get("addBook"):
            ...
            if "" in set(list_of_required_fields):
                ...
                if info_from_db is False:
                    ...
                    db_operations.save_temporary_info(
        ...
        if request.POST.get("searchInGB"):
            ...
            if set(list(query_d.values())) == {""}:
                ...
                if response is False:
                    info = "error"
                    messages.error(
                        request, "Wystąpił błąd przy połączeniu z Google Books"
                    )
                    return render(
                        request,
                        "gbapi/import_from_API.html",
                        {
                            "empty_table": db_operations.check_if_empty_table(),
                            "info": info,
                            "books": books,
                ...
                if db_info is False:
                    ...
                    if len(db_info) < len(response["add_title"]):
                        ...
                        try:
                            ...
                            info = (
                                "Pomyślnie zapisano "
                                + str(len(sorted_book_info[1]["add_title"]))
                                + " książek"
                            )
...
def confirm_overwriting(request):
    ...
    if books_info_str is None or l_isbn_str is None:
        ...
        return render(
            request,
            "gbapi/confirm_overwriting.html",
            {
...
def api_request(request):
    ...
    if (
        db_operations.check_if_empty_table() is True
        or set(arguments_l) in invalid_sets
        or correct_date is False
    ):

        return JsonResponse(
            {"results": "0", "api_response": "Nothing was found"}, status=404
        )
    else:
        ...
        if response is False:
            ...
            return JsonResponse(
                {"results": str(len(response)), "api_response": data}, status=200
            )
